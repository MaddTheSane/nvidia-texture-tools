...
static void convert_to_rgbm(const float4 input_colors[16], const float input_weights[16], float min_m, float4 rgbm_colors[16], float rgb_weights[16]) {
    ...
    for (uint i = 0; i < 16; i++) {
        ...
        rgbm_colors[i] = make_float4(r, g, b, a);
...
float nv::compress_dxt5_rgbm(const float4 input_colors[16], const float input_weights[16], float min_m, BlockDXT5 * output) {
    ...
    // Compress RGB.
    compress_dxt1(input_colors_rgbm, rgb_weights, 1, /*three_color_mode=*/false, &output->color);
...
float nv::compress_etc2_rgbm(float4 input_colors[16], float input_weights[16], float min_m, void * output) {
    
    // Convert to RGBM.
    float4 rgbm_colors[16];
    float rgb_weights[16];
    ...
    // Compress RGB.
    compress_etc2(rgbm_colors, rgb_weights, float3(1), etc_output);
